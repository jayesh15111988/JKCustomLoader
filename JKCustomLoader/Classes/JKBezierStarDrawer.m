...
- (NSArray*)polygonPointArrayWithSide:(NSInteger)sides
				 andX:(CGFloat)x
				 andY:(CGFloat)y
			    andRadius:(CGFloat)radius
			andAdjustment:(CGFloat)adjustment {
...
- (CGPathRef)starPathWithX:(CGFloat)x
		      andY:(CGFloat)y
		 andRadius:(CGFloat)radius
		  andSides:(NSInteger)sides
	     andPointiness:(CGFloat)pointiness {
	CGFloat adjustment = (CGFloat)(360 / sides / 2);
	CGMutablePathRef path = CGPathCreateMutable ();
	NSArray* points =
	    [self polygonPointArrayWithSide:sides andX:x andY:y andRadius:radius andAdjustment:adjustment];

	NSValue* rawValue = points[0];
	CGPoint cpg = [rawValue CGPointValue];
	NSArray* points2 = [self polygonPointArrayWithSide:sides
						      andX:x
						      andY:y
						 andRadius:(radius * pointiness)
					     andAdjustment:(CGFloat)adjustment];
	NSInteger i = 0;
...
- (UIBezierPath*)drawStarBezierWithX:(CGFloat)x
				andY:(CGFloat)y
			   andRadius:(CGFloat)radius
			    andSides:(NSInteger)sides
		       andPointiness:(CGFloat)pointiness {
